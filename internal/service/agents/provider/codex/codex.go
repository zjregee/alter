...
func NewCodexAgent(cfg agents.Config) (*CodexAgent, error) {
	if cfg.WorkDir == "" {
	...
	return &CodexAgent{
		id:     agents.GenerateAgentID(string(agents.TypeCodexAgent)),
		config: cfg,
		status: agents.Status{
...
func (c *CodexAgent) ID() string {
	return c.id
}

func (c *CodexAgent) Config() agents.Config {
	return c.config
}

func (c *CodexAgent) Execute(ctx context.Context) error {
	c.mu.Lock()
	...
	c.cmd = exec.CommandContext(taskCtx, "codex", args...)
...
func (c *CodexAgent) Cancel(ctx context.Context) error {
	c.mu.Lock()
...
func (c *CodexAgent) Done() <-chan struct{} {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *CodexAgent) Status() agents.Status {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *CodexAgent) buildCodexArgs() []string {
	args := []string{
		"-p", c.config.Prompt,
